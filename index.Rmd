...
# d-connection
dp1 <- ggdag_ov %>%
  ggdag_dconnected() + labs(title = "Omitted variable confounding")
...
# figure
fig1 <-
  dp1 + dp2 + plot_annotation(tag_levels = 'a')# + plot_layout(guides = 'collect')
...
pdm <-
  ggdag::ggdag_dconnected(dag_1) + labs(title = "Y ~ X (no Z)")
pdm2 <-
  ggdag::ggdag_dconnected(dag_1, controlling_for = "Z") + labs(title = "Y ~ X + Z")
...
# figure
fig2 <-
  pdm + pdm2 + pdm3 + plot_annotation(tag_levels = 'a',
                                      title = "Post-treatment confounding",
                                      subtitle = "When the causal effect of x on y is fully mediated, controlling for a mediator will block the path between x and y")  #+ plot_layout(guides = 'collect')
...
ddm2 <- ggdag::ggdag_dconnected( dag_3, controlling_for = "Z") + labs(title= "Y ~ X + Z")
...
dg_suz <- ggdag::dagify(
  ...
  labels = c(
    ...
    "sr" = "source",
    "st" = "statement",
    "im" = "importance of statement",
    "mem" = "memory of statement",
    "s" = "source of statement",
    "rel" = "participant's religiosity",
    "cny" = "participant's country",
    "mem" = "memory of statement",
    "male" = "male",
    "ordr" = "presentation order",
    "ses" = "perceived social economic status",
    "edu" = "education",
    "age" = "age"
  )
) %>%
...
dd4 <- ggdag::ggdag_collider(dg_suz, text = FALSE, use_labels = "label") + labs(title = "Colliders in experiment", "subtitle" = "Experimental control variables may induce collider bias from their causal relationships with each other.")
                                                                
dd5 <- ggdag::ggdag_adjustment_set(dg_suz, exposure = "rel", outcome = "b", text = FALSE, use_labels = "label") + labs(title = "Adjustment sets to block backdoorpaths", subtitle = "Two adjustment sets available to estimate source on credibility stratifying by religion.")
                                    
dd5 + labs(title = "Adjustment set", subtitle = "To estimate the causal effect of source on statemetn credibility only condition on religion and country.")                                                            
                                                                
# figure


fig3 <-(ddm + ddm2 + ddm3)/(dd4 + dd5) + plot_annotation(tag_levels = 'a', title = "Collider Bias")# + plot_layout(guides = 'collect')
...
dag_sd <- dagify(
  ...
  D ~ Z,
  labels = c(
    "Z" = "Collider (e.g. Distress)",
    "D" = "Descendant (e.g. Friends)",
    "X" = "Exposure (e.g. Religion)",
    "Y" = "Outcome (e.g. Meaning)"
  ),
  exposure = "X",
  outcome = "Y"
) %>%
...
ds2 <- dag_sd %>%
  ...
  ggtitle("Meaning ~ Religion, controlling for Friends",
          subtitle = "Friends induces collider bias because it is a descendent of Distress")
...
fig4 <-
  ds1 + ds2 + ds3 + plot_annotation(tag_levels = 'a',
                                    title = "Descendent confounding",
                                    subtitle = "Do not condition on the descendent of a collider")  #+ plot_layout(guides = 'collect')
