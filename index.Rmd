---
title: "The perils of “control:” why causal identification requires care"
description: ""
author:
...
p1 <- d1 + labs(title = "Omitted variable confounding")
p1
```
...
# plot the DAG
ggdag_ov %>%
  ggdag() + labs(title = "Omitted variable DAG")
```
...
'Backdoor Paths Unconditionally Closed' means that, assuming the DAG we have drawn is correct, we may obtain an unbiased estimate for the total effect of X on Y without including additional variables. 
...
library(patchwork)
p2 <- pdm + pdm2 + plot_annotation(tag_levels = 'a', title = "Post-treatment confounding", subtitle = "When the causal effect of x on y is fully mediated, controlling for a mediator will block the path between x and y")
p2 
...
```{r}
p3 <- collider_triangle() %>% 
  ggdag_dseparated(controlling_for = "m") 

p3 + labs(title = "Collider confounding")
```
